...
type HappyPathViewerVisual = { kind: "happy-path-viewer"; phase: "noisy" | "clean" };
...
.happy-viewer{background:#fff}.happy-viewer-controls{display:flex;align-items:center;gap:7px;padding:10px 12px;border-bottom:1px solid var(--line)}.happy-viewer-controls button{height:30px;border:1px solid var(--line-strong);border-radius:5px;background:#fff;padding:0 12px;cursor:pointer}.happy-viewer-controls .play{background:var(--ink);border-color:var(--ink);color:#fff}.happy-viewer-controls span{margin-left:auto;color:var(--muted);font-size:9px}.happy-viewer .matrix{min-width:620px}.happy-viewer.clean .bead.done{width:10px;height:10px;margin:-5px;background:var(--green)}.happy-viewer.clean .bead.done:after{content:none}.happy-viewer.clean .bead.current{border-color:var(--ink);box-shadow:0 0 0 2px var(--neutral-soft)}.happy-viewer.noisy .bead.done{background:var(--ink)}.happy-viewer.noisy .bead.done:after{content:"✓";position:absolute;inset:-1px 0 0;color:#fff;text-align:center;font-size:8px;font-style:normal;font-weight:800}.happy-viewer.noisy .bead.current.waiting{border-color:var(--attention);box-shadow:0 0 0 2px var(--attention-soft)}.happy-viewer.noisy .bead.current.failed{border-color:var(--danger);box-shadow:0 0 0 2px var(--danger-soft)}
...
function happyPathViewerVisual(snapshot){const clean=snapshot.visual.phase==="clean",view=clean?{...snapshot,blobs:snapshot.blobs.map(blob=>({...blob,state:blob.state==="complete"?"complete":"quiet"}))}:snapshot;return '<section class="happy-viewer '+(clean?"clean":"noisy")+'"><div class="happy-viewer-controls"><button class="play" data-happy-viewer-action="play" '+(clean?"disabled":"")+'>'+(clean?"Corrected":"Play correction")+'</button><button data-happy-viewer-action="reset">Reset</button><span>'+(clean?"Progress first · quiet happy path":"Before · noisy technical state")+'</span></div>'+matrix(view)+'</section>'}
