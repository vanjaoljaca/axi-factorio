...
type ViewSnapshot = {
  ...
  visual?: HappyPathVisual | HappyPathViewerVisual | LiveExecutionVisual | ViewerResilienceVisual | CursorActionVisual | LocalEndpointVisual | OverviewBoundaryVisual | ProjectRemovalVisual | AggregatePollingVisual | ActiveProjectsVisual | AxiValidationVisual | ServiceRecoveryVisual;
};
...
type HappyPathViewerVisual = { kind: "happy-path-viewer"; phase: "collapsed" | "expanded"; projects: string[] };
...
const happyPathPipsScenario = new HappyPathPipsScenario();
const happyPathViewerScenario = new HappyPathViewerScenario();
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    if (url.pathname === "/api/happy-path-pips/play" && request.method === "POST") {
      return json(response, await happyPathPipsScenario.play());
    }
    if (url.pathname === "/api/happy-path-pips/reset" && request.method === "POST") {
      return json(response, happyPathPipsScenario.reset());
    }
    if (url.pathname === "/api/happy-path-viewer/play" && request.method === "POST") {
      return json(response, happyPathViewerScenario.play());
    }
    if (url.pathname === "/api/happy-path-viewer/reset" && request.method === "POST") {
      return json(response, happyPathViewerScenario.reset());
    }
...
function scenarioIndex(): object[] {
  return [
    {
      id: "happy-path-pips", category: "Conveyor", name: "Happy-path pips",
      description: "Play · work pips · empty human pips · targeted feedback · fan-in · Reset",
    },
    {
    ...
    {
      id: "happy-path-viewer", category: "Viewer", name: "Happy-path pipeline rows",
      description: "Play · remove check badges and technical labels · sort tasks by progress · Reset",
    },
    {
...
async function scenario(url: URL): Promise<Scenario> {
  ...
  if (id === "happy-path-pips") return happyPathPipsScenario.snapshot() as unknown as Scenario;
  ...
  if (id === "happy-path-viewer") return happyPathViewerScenario.snapshot() as unknown as Scenario;
...
.happy-pips{background:#fff}.happy-pips-controls{display:flex;align-items:center;gap:7px;padding:10px 12px;border-bottom:1px solid var(--line)}.happy-pips-controls button{height:31px;border:1px solid var(--line-strong);border-radius:5px;background:#fff;padding:0 12px;cursor:pointer}.happy-pips-controls .play{background:var(--ink);border-color:var(--ink);color:#fff;font-weight:700}.happy-pips-controls .phase{margin-left:auto;color:var(--muted);font-size:9px;text-transform:uppercase;letter-spacing:.05em}.happy-pips-feedback{display:flex;align-items:center;gap:8px;padding:8px 12px;background:var(--attention-soft);color:#7c531e;font-size:9px}.happy-pips-feedback b{font-size:8px;text-transform:uppercase;letter-spacing:.05em}.happy-pips-artifacts{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;padding:12px;background:#f8faf8;border-top:1px solid var(--line)}.happy-artifact{min-height:138px;border:1px solid var(--line);background:#fff;padding:11px}.happy-artifact.missing{display:grid;place-items:center;color:var(--quiet);border-style:dashed}.happy-artifact>small{display:block;color:var(--muted);font-size:8px;text-transform:uppercase;letter-spacing:.05em}.happy-artifact h3{margin:14px 0 4px;font-size:17px;letter-spacing:-.04em}.happy-artifact p{margin:0;color:var(--muted);font-size:9px}.happy-artifact .artifact-kicker{margin-top:11px;color:var(--green);font-weight:700}.happy-artifact .artifact-chips{display:flex;gap:5px;flex-wrap:wrap;margin-top:12px}.happy-artifact .artifact-chips span{border:1px solid var(--line);border-radius:999px;padding:3px 7px;color:#505b55;font-size:8px}.happy-artifact .artifact-stat{margin-top:14px;font-size:22px;font-weight:800;letter-spacing:-.05em}.happy-artifact .artifact-facts{display:grid;grid-template-columns:repeat(3,1fr);gap:5px;margin-top:12px}.happy-artifact .artifact-facts span{padding:7px;background:#f4f6f4;text-align:center;font-size:8px}.happy-pips-meta{display:flex;gap:14px;padding:8px 12px;border-top:1px solid var(--line);color:var(--muted);font-size:8px}.happy-pips-meta b{color:var(--ink)}@media(max-width:760px){.happy-pips-artifacts{grid-template-columns:1fr}.happy-pips-controls .phase{display:none}}
${liveExecutionStyles}
...
.happy-viewer{min-width:620px;background:#fff}.happy-viewer-controls{display:flex;align-items:center;gap:7px;padding:10px 12px;border-bottom:1px solid var(--line)}.happy-viewer-controls button{height:30px;border:1px solid var(--line-strong);border-radius:5px;background:#fff;padding:0 12px;cursor:pointer}.happy-viewer-controls .play{background:var(--ink);border-color:var(--ink);color:#fff}.happy-viewer-controls span{margin-left:auto;color:var(--muted);font-size:9px}.happy-project-card+.happy-project-card{border-top:1px solid var(--line)}.happy-project-head{height:37px;display:grid;grid-template-columns:280px repeat(var(--steps),minmax(72px,1fr)) 36px;align-items:center}.happy-project-name{height:37px;display:flex;align-items:center;padding:0 24px;font-weight:700;font-size:10px}.happy-project-stage{height:37px;position:relative}.happy-project-stage:before{content:"";position:absolute;left:0;right:0;top:18px;height:1px;background:var(--line-strong)}.happy-project-stage.first:before{left:50%}.happy-project-stage.last:before{right:50%}.happy-project-stage .aggregate-bead{position:absolute;z-index:1;left:50%;top:50%;width:16px;height:16px;margin:-8px;border-radius:50%;background:var(--composition);box-shadow:0 0 0 1px #fff,0 0 0 2px #aeb7b1}.happy-project-stage .aggregate-bead:after{content:"";position:absolute;inset:4px;border-radius:50%;background:#fff}.happy-project-toggle{height:37px;border:0;border-left:1px solid var(--line);background:#fff;color:var(--muted);font-size:17px}.happy-project-card .taskrows{border-top:1px solid var(--line)}.happy-viewer .bead.done{width:10px;height:10px;margin:-5px;background:var(--green)}.happy-viewer .bead.done:after{content:none}.happy-viewer .bead.current{border-color:var(--ink);box-shadow:0 0 0 2px var(--neutral-soft)}
...
function renderScenario(){const snapshot=frames[frame];if(!snapshot)return;const interactive=["happy-pips","happy-path-viewer","live-execution","cursor-action","local-endpoint","project-removal","aggregate-polling","active-projects","axi-validation","service-recovery"].includes(snapshot.visual?.kind);byId("title").textContent=snapshot.name;byId("description").textContent=snapshot.description;byId("frame").textContent=interactive?"Live temporary state":source==="scenario"?"Frame "+(frame+1)+" / "+frames.length:"Live database";byId("run").hidden=source!=="scenario"||interactive;byId("run").textContent="Run scenario";showFrameControls(source==="scenario"&&!interactive);byId("workspace").innerHTML=scenarioVisual(snapshot);renderEvidence(snapshot);byId("total").textContent=snapshot.visual?.kind==="axi-validation"?"Official 10-principle contract":snapshot.blobs.length+" blob"+(snapshot.blobs.length===1?"":"s")+" · "+snapshot.receipts.length+" receipts";if(snapshot.visual?.kind==="happy-pips")wireHappyPips();if(snapshot.visual?.kind==="happy-path-viewer")wireHappyPathViewer();if(snapshot.visual?.kind==="live-execution")wireLiveExecution(snapshot.visual);if(snapshot.visual?.kind==="cursor-action")wireCursorAction();if(snapshot.visual?.kind==="local-endpoint")wireLocalEndpoint();if(snapshot.visual?.kind==="project-removal")wireProjectRemoval();if(snapshot.visual?.kind==="aggregate-polling")wireAggregatePolling();if(snapshot.visual?.kind==="active-projects")wireActiveProjects();if(snapshot.visual?.kind==="axi-validation")wireAxiValidation(snapshot.visual);if(snapshot.visual?.kind==="service-recovery")wireServiceRecovery()}
function scenarioVisual(snapshot){if(snapshot.visual?.kind==="happy-pips")return happyPipsVisual(snapshot);if(snapshot.visual?.kind==="happy-path-viewer")return happyPathViewerVisual(snapshot);if(snapshot.visual?.kind==="service-recovery")return serviceRecoveryVisual(snapshot);if(snapshot.visual?.kind==="axi-validation")return axiValidationVisual(snapshot.visual);if(snapshot.visual?.kind==="live-execution")return liveExecutionVisual(snapshot);if(snapshot.visual?.kind==="cursor-action")return cursorActionVisual(snapshot);if(snapshot.visual?.kind==="local-endpoint")return localEndpointVisual(snapshot);if(snapshot.visual?.kind==="project-removal")return projectRemovalVisual(snapshot);if(snapshot.visual?.kind==="aggregate-polling")return aggregatePollingVisual();if(snapshot.visual?.kind==="active-projects")return activeProjectsVisual(snapshot.visual);if(snapshot.visual?.kind==="overview-boundary")return overviewBoundaryVisual(snapshot);if(snapshot.visual?.kind==="viewer-resilience")return viewerResilienceVisual(snapshot)+matrix(snapshot)+scenarioEvidence(snapshot);return matrix(snapshot)+scenarioEvidence(snapshot)}
...
function happyPathViewerVisual(snapshot){const expanded=snapshot.visual.phase==="expanded",view={...snapshot,blobs:snapshot.blobs.map(blob=>({...blob,state:blob.state==="complete"?"complete":"quiet"}))},projects=snapshot.visual.projects.map((name,index)=>happyPathProject(view,name,index,expanded&&index===1)).join("");return '<section class="happy-viewer"><div class="happy-viewer-controls"><button class="play" data-happy-viewer-action="play" '+(expanded?"disabled":"")+'>'+(expanded?"Expanded":"Expand Beta project")+'</button><button data-happy-viewer-action="reset">Reset</button><span>Projects A–Z · tasks progress first</span></div>'+projects+'</section>'}
function happyPathProject(snapshot,name,index,expanded){const counts=[[1,0,0,0],[3,2,1,0],[4,4,4,4]][index],track=snapshot.steps.map((step,stepIndex)=>'<div class="happy-project-stage '+(stepIndex===0?'first ':'')+(stepIndex===snapshot.steps.length-1?'last':'')+'"><i class="aggregate-bead" style="--composition:'+aggregateProgressGradient(counts[stepIndex],4)+'" title="'+counts[stepIndex]+' of 4 completed"></i></div>').join(""),rows=expanded?'<div class="taskrows">'+snapshot.blobs.map(blob=>taskRow(blob,snapshot.steps)).join("")+'</div>':"";return '<section class="happy-project-card"><div class="happy-project-head" style="--steps:'+snapshot.steps.length+'"><div class="happy-project-name">'+safe(name)+'</div>'+track+'<button class="happy-project-toggle" aria-label="'+(expanded?'Collapse':'Expand')+' '+safe(name)+'">'+(expanded?'⌄':'‹')+'</button></div>'+rows+'</section>'}
function wireHappyPathViewer(){document.querySelectorAll("[data-happy-viewer-action]").forEach(button=>button.onclick=()=>happyPathViewerAction(button.dataset.happyViewerAction))}
async function happyPathViewerAction(action){byId("error").textContent="";const response=await fetch("/api/happy-path-viewer/"+action,{method:"POST"});if(!response.ok)return showScenarioError(await response.json());frames=(await response.json()).frames;frame=0;renderScenario()}
function happyPipsVisual(snapshot){const data=snapshot.visual,done=data.phase==="complete",feedback=data.feedback?'<div class="happy-pips-feedback"><b>Human feedback</b><span>'+safe(data.feedback)+'</span></div>':"";return '<section class="happy-pips"><div class="happy-pips-controls"><button class="play" data-happy-action="play" '+(done?'disabled':'')+'>'+safe(happyPipsActionLabel(data.phase))+'</button><button data-happy-action="reset">Reset</button><span class="phase">'+safe(data.phase)+'</span></div>'+feedback+'<div class="happy-pips-track">'+matrix(snapshot)+'</div><div class="happy-pips-artifacts">'+data.artifacts.map(happyPipsArtifact).join("")+'</div><div class="happy-pips-meta"><span><b>'+data.harnessCalls+'</b> work-pip harness runs</span><span><b>'+snapshot.receipts.filter(item=>item.status!=="invalidated").length+'</b> valid receipts</span><span>Empty human pips allocate no agent</span></div></section>'}
function happyPipsActionLabel(phase){return {structure:"Play structure pip",presentation:"Play presentation pip",choose:"Add feedback + rewind",feedback:"Play revised presentation",revised:"Approve choice",merge:"Play merge pip",accept:"Approve result",complete:"Complete"}[phase]||"Play next pip"}
function happyPipsArtifact(artifact){if(!artifact.present)return '<article class="happy-artifact missing"><span>'+safe(artifact.label)+' appears here</span></article>';let data={};try{data=JSON.parse(artifact.content)}catch{}if(artifact.id==="structure")return '<article class="happy-artifact"><small>'+safe(artifact.label)+'</small><p class="artifact-kicker">'+safe(data.eyebrow||"")+'</p><h3>'+safe(data.title||"")+'</h3><div class="artifact-chips">'+(data.sections||[]).map(item=>'<span>'+safe(item)+'</span>').join("")+'</div></article>';if(artifact.id==="presentation")return '<article class="happy-artifact"><small>'+safe(artifact.label)+'</small><div class="artifact-stat">'+safe(data.columns||"—")+' columns</div><p>'+safe(data.density||"")+' density · '+safe(data.accent||"")+' accent</p><div class="artifact-chips"><span>'+safe(data.note||"")+'</span></div></article>';return '<article class="happy-artifact"><small>'+safe(artifact.label)+'</small><h3>'+safe(data.title||"")+'</h3><div class="artifact-stat">'+safe(data.primary||"")+'</div><div class="artifact-facts">'+(data.secondary||[]).map(item=>'<span>'+safe(item)+'</span>').join("")+'</div></article>'}
function wireHappyPips(){document.querySelectorAll('[data-happy-action]').forEach(button=>button.onclick=()=>happyPipsAction(button.dataset.happyAction))}
async function happyPipsAction(action){byId("error").textContent="";const response=await fetch("/api/happy-path-pips/"+action,{method:"POST"});if(!response.ok){const error=await response.json().catch(()=>({error:"Happy-path action failed."}));byId("error").textContent=error.error;return}const data=await response.json();frames=data.frames;frame=0;renderScenario()}
...
import { HappyPathPipsScenario, type HappyPathVisual } from "../test/harness/HappyPathPipsScenario.ts";
