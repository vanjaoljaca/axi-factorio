...
type ViewSnapshot = {
  ...
  visual?: LiveExecutionVisual;
};
type Scenario = { id: string; frames: ViewSnapshot[] };
type LiveExecutionVisual = {
  kind: "live-execution";
  phase: "ready" | "queued" | "running" | "complete" | "failed";
...
function scenarioIndex(): object[] {
  return [
    ...
    {
      id: "live-execution-visibility", name: "Live agent sessions",
      description: "Play · watch Store/Runner activity · complete · Reset",
    },
  ];
}
...
function liveExecutionVisual(data){const attempts=data.executions.filter(item=>!item.stale);const retried=attempts.some(item=>item.status==="retry");const nodes=retried?[["ready","Ready"],["queued","Queued"],["run1","Running #1"],["retry","Retry #1"],["run2","Running #2"],["complete","Complete"]]:[["ready","Ready"],["queued","Queued"],["running","Agent running"],["retry","Retry"],["complete","Complete"]];const current=retried?(data.phase==="retry"?"retry":data.phase==="running"?"run2":data.phase):data.phase;const currentIndex=nodes.findIndex(([id])=>id===current);const track=nodes.map(([id,label],position)=>'<div class="activity-node '+(position<currentIndex?"done":position===currentIndex?"current":"")+'">'+label+'</div>').join("");const log=data.timeline.slice(-5).map(item=>'<span>#'+item.id+" · "+safe(item.label)+'</span>').join("");return '<section class="live-scenario"><div class="live-scenario-controls"><button class="play" data-live-action="play" '+(data.playEnabled?"":"disabled")+'>Play</button><button data-live-action="reset">Reset</button><span>'+safe(data.phase)+'</span></div>'+data.executionOverviewHtml+'<div class="activity-track">'+track+'</div><div class="activity-log">'+(log||"<span>Play to start the real runner.</span>")+"</div></section>"}
