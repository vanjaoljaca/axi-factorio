...
type ViewSnapshot = {
  ...
  visual?: AgentGitCommitVisual;
};
type Scenario = { id: string; frames: ViewSnapshot[] };
type AgentGitCommitVisual = {
  kind: "git-commit";
...
function scenarioIndex(): object[] {
  return [
    ...
    {
      id: "agent-git-commit-boundary", name: "Agent Git commit boundary",
      description: "assigned workspace + narrow Git metadata writes + real commit",
    },
  ];
}
...
async function scenario(url: URL): Promise<Scenario> {
  ...
  if (id === "agent-git-commit-boundary") {
    const { runAgentGitCommitBoundaryScenario } =
      await import("../test/harness/AgentGitCommitBoundaryScenario.ts");
    return runAgentGitCommitBoundaryScenario();
  }
...
function scenarioEvidence(snapshot){const visual=snapshot.visual?.kind==="git-commit"?agentGitCommitVisual(snapshot.visual):"",cards=snapshot.evidenceCards?.length?'<details class="scenario-debug"><summary>Exact evidence · argv, paths and JSON</summary><div class="evidence-grid">'+snapshot.evidenceCards.map(card=>evidenceCard(card.label,"",card.value,card.label.includes("argv"))).join("")+"</div></details>":"";return visual+cards}
function agentGitCommitVisual(data){const changed=data.beforeHead!==data.afterHead,decision=String(data.decision);return '<section class="scenario-visual"><div class="visual-head"><strong>Agent Git commit boundary</strong><span class="'+safe(decision)+'">'+safe(decision)+'</span></div><div class="boundary-layout"><div class="workspace-boundary"><div class="boundary-label"><b>Assigned execution workspace · sandbox</b><span>workspace/</span></div><div class="root-pair"><div class="visual-root primary"><small>Project root · app identity</small><b>'+safe(relativePath(data.executionRoot,data.projectRoot))+'/</b><div class="file-effect">'+(data.files.app?"app fixture changed":"app fixture unchanged")+'</div></div><div class="visual-root"><small>Sibling Workbench</small><b>'+safe(relativePath(data.executionRoot,data.siblingRoot))+'/</b><div class="file-effect">'+(data.files.sibling?"fixture changed":"fixture unchanged")+'</div></div></div><div class="commit-track"><div class="commit-node"><small>Before</small><b>'+safe(shortSha(data.beforeHead))+'</b></div><div class="commit-arrow"></div><div class="commit-node"><small>'+(changed?"Committed":"Unchanged")+'</small><b>'+safe(shortSha(data.afterHead))+'</b></div></div><div class="escape-proof">'+(data.files.outside?"× Write escaped boundary":"✓ No write escaped the assigned workspace or Git metadata")+'</div></div><aside class="git-boundary"><div class="boundary-label"><b>Git-owned writes</b><span>'+data.writableDirs.length+' dirs</span></div><div class="git-path"><small>Repository state</small><b>work state</b></div><div class="git-path"><small>Shared stores</small><b>objects · refs · logs</b></div></aside></div></section>'}
