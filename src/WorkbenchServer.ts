...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    if (url.pathname.startsWith("/api/local-endpoint/") && request.method === "POST") {
      const action = url.pathname.split("/").at(-1) as "approve" | "reject" | "restart" | "recover" | "reset";
      return json(response, await (action === "recover"
        ? localEndpointScenario.recoverLostChild()
        : localEndpointScenario[action]()));
    }
...
${liveExecutionStyles}
.endpoint-flow{grid-template-columns:repeat(7,1fr)}
...
function activeProjectsAction(action){if(action==='toggle'){const hidden=byId('active-projects-table').querySelector('[data-inactive-projects]'),button=byId('active-projects-table').querySelector('[data-active-action="toggle"]'),show=hidden.hidden;hidden.hidden=!show;button.setAttribute('aria-expanded',String(show));button.innerHTML='<span aria-hidden="true">'+(show?'⌃':'⌄')+'</span>'+(show?'Hide inactive projects':'Show all projects')+' <small>'+hidden.children.length+'</small>';return}if(action==='reset'){clearInterval(activeProjectRefreshTimer);activeProjectRefreshes=0;byId('active-projects-health').textContent='Active rule: at least one nonterminal task';return}clearInterval(activeProjectRefreshTimer);const projects=[...document.querySelectorAll('[data-active-project]')];activeProjectRefreshTimer=setInterval(()=>{activeProjectRefreshes++;const stable=projects.every((node,index)=>node===document.querySelectorAll('[data-active-project]')[index]);byId('active-projects-health').textContent=activeProjectRefreshes+' refreshes · '+(stable?'same project nodes · fold preserved':'node replaced');if(activeProjectRefreshes>=6)clearInterval(activeProjectRefreshTimer)},500)}
function demoComposition(index){const colors=index<2?['var(--green)','var(--green)','var(--green)','var(--ink)','#aeb7b1','#aeb7b1']:index===2?['var(--green)','var(--attention)','#aeb7b1','#aeb7b1','#aeb7b1','#aeb7b1']:['#aeb7b1','#aeb7b1','#aeb7b1','#aeb7b1','#aeb7b1','#aeb7b1'];return segmentedDemo(colors)}
function segmentedDemo(colors){const span=360/colors.length,gap=2,stops=[];colors.forEach((color,index)=>{const start=index*span,end=(index+1)*span;stops.push('#fff '+start+'deg '+(start+gap)+'deg',color+' '+(start+gap)+'deg '+(end-gap)+'deg','#fff '+(end-gap)+'deg '+end+'deg')});return 'conic-gradient(from -90deg,'+stops.join(',')+')'}
function demoLabel(stage,index){return stage+' — 6 tasks: '+(index<2?'3 completed, 1 running, 0 need attention, 0 failed, 2 unfinished or inventory':index===2?'1 completed, 0 running, 1 need attention, 0 failed, 4 unfinished or inventory':'0 completed, 0 running, 0 need attention, 0 failed, 6 unfinished or inventory')}
