...
<main class="content"><div class="toolbar"><div class="scenario-copy"><strong id="title">Loading scenario</strong><span id="description"></span></div><select class="picker" id="scenario-picker" aria-label="Choose scenario" hidden></select><select class="picker" id="test-picker" aria-label="Choose test" hidden></select><span class="frame" id="frame"></span></div><div class="workspace" id="workspace"><div class="empty">Loading scenario…</div></div><div class="footer"><div class="legend"><span><i class="key complete"></i>Completed</span><span><i class="key imported"></i>Imported</span><span><i class="key inventory"></i>Inventory</span><span><i class="key current"></i>Current</span><span><i class="key waiting"></i>Awaiting review / needs attention</span><span><i class="key"></i>Pending</span><span><i class="key failed"></i>Failed</span></div><span class="total" id="total"></span></div><details class="inspector" open><summary><span>Inspect evidence</span><small id="result"></small></summary><div class="evidence"><section class="panel"><div class="panel-head"><span id="event-label">Receipt stream</span><span>append only</span></div><div id="events"></div></section><section class="panel"><div class="panel-head"><span>Assertions</span><span id="assertion-count"></span></div><div id="checks"></div></section></div></details><div id="error" role="status"></div></main></div>
<script>
let source="lab",selected="happy",selectedTest="",scenarios=[],tests=[],testRun=null,frames=[],frame=0,timer,loadVersion=0,lab=null;
...
function renderLab(){const receipt=lab.receipts.at(-1),snapshot={name:lab.name,description:lab.description,steps:lab.steps,blobs:[{id:lab.blob.id,title:lab.blob.title,state:lab.blob.paused?(receipt?.status==="failed"?"failed":"waiting"):(lab.blob.state==="complete"?"complete":receipt?.status==="running"?"running":"ready"),stepId:lab.blob.state}],receipts:lab.receipts.map(item=>({at:(item.finishedAt||item.startedAt).slice(11,19),stepId:item.stepId,status:item.status,detail:"attempt "+item.attempt+" · in ["+item.inputArtifacts.join(", ")+"] · out ["+item.outputArtifacts.join(", ")+"]"})),assertions:lab.assertions};byId("scenario-picker").hidden=true;byId("test-picker").hidden=true;byId("title").textContent=lab.name;byId("description").textContent=lab.description;byId("frame").textContent="Live production boundary";byId("run").hidden=true;showFrameControls(false);byId("workspace").innerHTML='<div class="lab-actions">'+["play","step","stop","retry","feedback","approve","fail","restart","reset"].map(labButton).join("")+'</div>'+matrix(snapshot)+'<div class="lab-facts"><div class="lab-fact"><small>Blob state</small><b>'+safe(lab.blob.state)+'</b></div><div class="lab-fact"><small>Mode / queued</small><b>'+safe(lab.blob.executionMode+" / "+lab.blob.runRequested)+'</b></div><div class="lab-fact"><small>Latest attempt</small><b>'+safe(receipt?receipt.stepId+" #"+receipt.attempt+" "+receipt.status:"none")+'</b></div><div class="lab-fact"><small>Latest / resumable run</small><b>'+safe(lab.latestExternalRunId||"none")+'</b></div><div class="lab-fact"><small>Artifacts</small><b>'+safe(receipt?[...receipt.inputArtifacts,...receipt.outputArtifacts].join(", "):"request:mock-lab")+'</b></div></div>';byId("event-label").textContent="Harness event timeline";byId("events").innerHTML=lab.events.length?lab.events.slice().reverse().map(event=>'<div class="event"><span>#'+event.id+'</span><b>'+safe(event.stepId)+'</b><b>'+safe(event.name.replace("axi_factorio.harness.",""))+'</b><span>'+safe(JSON.stringify(event.attributes))+'</span></div>').join(""):'<div class="empty">No execution events yet.</div>';byId("checks").innerHTML=lab.assertions.map(item=>'<div class="check"><b class="'+(item.passed?"pass":"fail")+'">'+(item.passed?"✓":"×")+'</b> '+safe(item.label)+'</div>').join("")+'<div class="check">'+lab.receipts.length+' receipts · '+lab.events.length+' append-only events · '+lab.humanInputs.length+' human inputs</div>';byId("assertion-count").textContent=lab.assertions.filter(item=>item.passed).length+" / "+lab.assertions.length;byId("result").textContent="Deterministic mock proof";byId("total").textContent=lab.receipts.length+" receipts · "+lab.events.length+" events";document.querySelectorAll("[data-lab-action]").forEach(button=>button.onclick=()=>labAction(button.dataset.labAction))}
function labButton(action){const labels={play:"Play continuously",step:"Run one transition",stop:"Stop"};const icons={play:'<svg viewBox="0 0 12 12" aria-hidden="true"><path d="M2 1.3v9.4L10 6z"/></svg>',step:'<svg viewBox="0 0 14 12" aria-hidden="true"><path d="M1 1.3v9.4L9 6zM11 1h2v10h-2z"/></svg>',stop:'<svg viewBox="0 0 12 12" aria-hidden="true"><path d="M2 2h8v8H2z"/></svg>'};return icons[action]?'<button class="run-control" data-lab-action="'+action+'" aria-label="'+labels[action]+'" title="'+labels[action]+'">'+icons[action]+'</button>':'<button data-lab-action="'+action+'">'+safe(action[0].toUpperCase()+action.slice(1))+'</button>'}
async function labAction(action){const target=document.querySelector('[data-lab-action="'+action+'"]');if(target)target.disabled=true;try{const response=await fetch("/api/mock-lab/"+action,{method:"POST"});if(!response.ok)throw new Error(await response.text());lab=await response.json();renderLab();if(action==="play")scheduleLabRefresh()}catch(error){byId("error").textContent=error.message}}
function scheduleLabRefresh(){[350,750,1200].forEach(delay=>setTimeout(async()=>{if(source!=="lab")return;lab=await fetch("/api/mock-lab").then(response=>response.json());renderLab()},delay))}
