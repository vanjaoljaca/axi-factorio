...
type ViewSnapshot = {
  ...
  visual?: GitWorktreeVisual;
};
type Scenario = { id: string; frames: ViewSnapshot[] };
type GitWorktreeVisual = {
  kind: "git-worktree";
  worktreeRoot: string;
  projectRoot: string;
  siblingRoot: string;
  ...
  afterHead: string;
  decision: string;
  files: { app: boolean; sibling: boolean; outside: boolean };
};
...
function scenarioIndex(): object[] {
  return [
    ...
    {
      id: "codex-git-worktree", name: "Codex linked-worktree commit",
      description: "worktree sandbox + exact Git metadata writes + real commit",
    },
  ];
}
...
async function scenario(url: URL): Promise<Scenario> {
  ...
  if (id === "codex-git-worktree") {
    const { runCodexGitWorktreeScenario } =
      await import("../test/harness/CodexGitWorktreeScenario.ts");
    return runCodexGitWorktreeScenario();
  }
...
.scenario-visual{padding:16px;border-top:1px solid var(--line);background:#fafbfa}.visual-head{display:flex;align-items:center;gap:10px;margin-bottom:12px}.visual-head strong{font-size:12px}.visual-head span{margin-left:auto;border:1px solid var(--line-strong);border-radius:999px;padding:3px 8px;font-size:9px;text-transform:uppercase}.visual-head span.advance{color:#24764e;background:#f0f8f3;border-color:#bcdcc9}.boundary-layout{display:grid;grid-template-columns:minmax(0,1fr) 210px;gap:12px}.worktree-boundary,.git-boundary{border:2px solid #8b948e;border-radius:9px;background:#fff;padding:12px}.git-boundary{border-style:dashed}.boundary-label{display:flex;justify-content:space-between;gap:8px;font-size:9px;color:var(--muted)}.boundary-label b{color:var(--ink);font-size:10px}.root-pair{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:10px}.visual-root{border:1px solid var(--line-strong);border-radius:6px;padding:10px;min-height:72px}.visual-root.primary{box-shadow:inset 0 2px 0 #3b7e5d}.visual-root small,.git-path small{display:block;color:var(--muted);font-size:8px}.visual-root b{display:block;margin-top:3px;font-size:10px}.file-effect{margin-top:9px;color:#24764e;font-size:9px}.file-effect:before{content:"✓";margin-right:5px;font-weight:800}.git-path{border:1px solid var(--line);border-radius:5px;padding:8px;margin-top:8px}.git-path b{font-size:9px}.commit-track{display:grid;grid-template-columns:1fr 42px 1fr;align-items:center;margin-top:12px}.commit-node{border:1px solid var(--line);border-radius:6px;padding:9px}.commit-node small{display:block;color:var(--muted);font-size:8px}.commit-node b{display:block;margin-top:3px;font:9px ui-monospace,SFMono-Regular,Menlo,monospace}.commit-arrow{height:1px;background:var(--line-strong);position:relative}.commit-arrow:after{content:"";position:absolute;right:0;top:-3px;width:6px;height:6px;border-top:1px solid var(--muted);border-right:1px solid var(--muted);transform:rotate(45deg)}.escape-proof{margin-top:10px;padding:8px;border:1px solid var(--line);border-radius:5px;font-size:9px;color:#24764e}.scenario-debug{border-top:1px solid var(--line);padding:10px;background:#fff}.scenario-debug summary{cursor:pointer;font-size:9px;font-weight:700;color:var(--muted)}.scenario-debug .evidence-grid{padding-top:9px}@media(max-width:900px){.boundary-layout,.root-pair{grid-template-columns:1fr}}
...
function renderScenario(){const snapshot=frames[frame];if(!snapshot)return;byId("title").textContent=snapshot.name;byId("description").textContent=snapshot.description;byId("frame").textContent=source==="scenario"?"Frame "+(frame+1)+" / "+frames.length:"Live database";byId("run").hidden=source!=="scenario";byId("run").textContent="Run scenario";showFrameControls(source==="scenario");byId("workspace").innerHTML=(snapshot.visual?"":matrix(snapshot))+scenarioEvidence(snapshot);renderEvidence(snapshot);byId("total").textContent=snapshot.blobs.length+" blob"+(snapshot.blobs.length===1?"":"s")+" · "+snapshot.receipts.length+" receipts"}
function scenarioEvidence(snapshot){const visual=snapshot.visual?.kind==="git-worktree"?gitWorktreeVisual(snapshot.visual):"",cards=snapshot.evidenceCards?.length?'<details class="scenario-debug"><summary>Exact evidence · argv, paths and JSON</summary><div class="evidence-grid">'+snapshot.evidenceCards.map(card=>evidenceCard(card.label,"",card.value,card.label.includes("argv"))).join("")+"</div></details>":"";return visual+cards}
function gitWorktreeVisual(data){const changed=data.beforeHead!==data.afterHead,decision=String(data.decision);return '<section class="scenario-visual"><div class="visual-head"><strong>Worktree commit boundary</strong><span class="'+safe(decision)+'">'+safe(decision)+'</span></div><div class="boundary-layout"><div class="worktree-boundary"><div class="boundary-label"><b>Execution workspace · sandbox</b><span>'+safe(lastPath(data.worktreeRoot))+'/</span></div><div class="root-pair"><div class="visual-root primary"><small>Project root · app identity</small><b>'+safe(relativePath(data.worktreeRoot,data.projectRoot))+'/</b><div class="file-effect">'+(data.files.app?"app fixture changed":"app fixture unchanged")+'</div></div><div class="visual-root"><small>Sibling Workbench</small><b>'+safe(relativePath(data.worktreeRoot,data.siblingRoot))+'/</b><div class="file-effect">'+(data.files.sibling?"fixture changed":"fixture unchanged")+'</div></div></div><div class="commit-track"><div class="commit-node"><small>Before</small><b>'+safe(shortSha(data.beforeHead))+'</b></div><div class="commit-arrow"></div><div class="commit-node"><small>'+(changed?"Committed":"Unchanged")+'</small><b>'+safe(shortSha(data.afterHead))+'</b></div></div><div class="escape-proof">'+(data.files.outside?"× Write escaped boundary":"✓ No write escaped the worktree or Git metadata")+'</div></div><aside class="git-boundary"><div class="boundary-label"><b>Additional Git writes</b><span>'+data.writableDirs.length+' dirs</span></div><div class="git-path"><small>Linked-worktree state</small><b>'+safe(gitLabel(data.gitDir,data.gitCommonDir))+'</b></div><div class="git-path"><small>Shared repository stores</small><b>objects · refs · logs</b></div></aside></div></section>'}
function lastPath(value){return String(value).split("/").filter(Boolean).at(-1)||value}
function relativePath(root,value){return String(value).startsWith(String(root)+"/")?String(value).slice(String(root).length+1):lastPath(value)}
function shortSha(value){return String(value||"missing").slice(0,10)}
function gitLabel(gitDir,commonDir){return relativePath(commonDir,gitDir)}
