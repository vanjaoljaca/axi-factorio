...
function groupProjects(blobs: Blob[], receipts: Receipt[], records: ProjectRecord[]): ViewProject[] {
  const projects = new Map<string, ViewProject>();
...
function render(){const query=byId('search').value.trim().toLowerCase();const projects=snapshot.projects.map(project=>({...project,blobs:project.blobs.filter(blob=>!query||project.name.toLowerCase().includes(query)||blob.title.toLowerCase().includes(query)||blob.id.toLowerCase().includes(query))})).filter(project=>project.blobs.length);byId('summary').textContent='1 command center · '+snapshot.stats.projects+' '+plural(snapshot.stats.projects,'project')+' · '+snapshot.stats.tasks+' '+plural(snapshot.stats.tasks,'task');byId('total').textContent=snapshot.stats.tasks+' '+plural(snapshot.stats.tasks,'task')+' total';byId('projects').innerHTML=projects.length?projects.map(projectCard).join(''):'<div class="no-results">'+(query?'No projects or tasks match your search.':'No tasks yet. New work will appear here automatically.')+'</div>'}
...
function sharedSteps(blobs){const longest=blobs.reduce((chosen,blob)=>blob.steps.length>chosen.length?blob.steps:chosen,[]);return longest.length?longest:[{id:'work',label:'Work'}]}
