...
const byId=id=>document.getElementById(id);let snapshot=null,learning=null,selectedAttemptId=null,blobPreview=null,promptPreview=null,currentPage='overview',menuTrigger=null,menuBlobId=null;const collapsedProjects=new Set(),knownProjects=new Set();
...
function renderOverview(){const projects=filteredProjects();const query=byId('search').value.trim();byId('total').textContent=snapshot.stats.tasks+' '+plural(snapshot.stats.tasks,'task')+' across '+snapshot.stats.projects+' '+plural(snapshot.stats.projects,'project');byId('workspace').innerHTML=projects.length?matrix(projects):'<div class="no-results">'+(query?'No projects or tasks match your search.':'No projects yet. Add a project and its work will appear here.')+'</div>'}
