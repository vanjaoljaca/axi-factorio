...
async function viewSettingsRequest(request: IncomingMessage, response: ServerResponse, databasePath: string): Promise<void> {
  ...
  try {
    ...
    const sortProjectsByProgress = store.setSortProjectsByProgress(false);
...
function groupProjects(
  ...
  for (const project of projects.values()) project.blobs = sortBlobs(project.blobs);
...
<div class="footer" id="footer"><div class="legend"><span><i class="key complete"></i>Completed</span><span><i class="key imported"></i>Imported</span><span><i class="key current"></i>Current</span><span><i class="key"></i>Pending</span></div><span class="total" id="total"></span></div>
...
function renderAlerts(){const alerts=[];for(const project of snapshot.projects){if(project.pipelineIssue)alerts.push({kind:'failure',title:project.name+' · Pipeline unavailable',detail:project.pipelineIssue.detail});if(snapshot.settings.debugMode)for(const blob of project.blobs){if(blob.status==='failed')alerts.push({kind:'failure',title:blob.title+' · Execution issue',detail:'Current pip: '+blob.stepId});else if(blob.status==='blocked'||blob.status==='waiting')alerts.push({kind:'attention',title:blob.title+' · Paused',detail:project.name+' · '+blob.stepId})}}byId('workspace').innerHTML=alerts.length?'<div class="page-list">'+alerts.map(alert=>'<article class="page-card alert-card '+alert.kind+'"><div class="page-card-head"><strong>'+escapeHtml(alert.title)+'</strong></div><p>'+escapeHtml(alert.detail)+'</p></article>').join('')+'</div>':'<div class="empty"><b>No pipeline issues</b>The happy-path view stays focused on progress.</div>'}
function renderSettings(){const enabled=snapshot.settings.debugMode,opener=snapshot.settings.opener,days=snapshot.settings.activeProjectDays;byId('workspace').innerHTML='<section class="page-list"><article class="page-card settings-card"><div class="setting-row"><div class="setting-copy"><strong>Default opener</strong><p>Choose the local app used by Open on a task.</p></div><select class="opener-select" id="opener" aria-label="Default opener"><option value="cursor" '+(opener.id==='cursor'?'selected':'')+'>Cursor</option></select></div></article><article class="page-card settings-card"><div class="setting-row"><div class="setting-copy"><strong>Active project window</strong><p>Show projects with meaningful task activity in the last number of days.</p></div><input class="view-days" id="active-project-days" type="number" min="1" max="365" value="'+days+'" aria-label="Active project window in days"></div></article><article class="page-card settings-card"><div class="setting-row"><div class="setting-copy"><strong>Debug mode</strong><p>Pause automatic progression and expose manual Step, Play, and Stop controls for pipeline validation.</p></div><label class="switch"><input id="debug-mode" type="checkbox" '+(enabled?'checked':'')+' aria-label="Debug mode"><span></span></label></div>'+(enabled?'<div class="debug-notice">Debug mode is on. Continuous play is disabled; use Step to advance one transition at a time.</div>':'')+'</article></section>'}
...
function aggregateModel(project,step){const tasks=project.blobs.filter(blob=>blob.steps.some(candidate=>candidate.id===step.id)).slice().sort((a,b)=>a.id.localeCompare(b.id)),counts={completed:0,running:0,attention:0,failed:0,unfinished:0};for(const blob of tasks)counts[aggregateCategory(blob,step)]++;const total=tasks.length,composition=total?aggregateProgressGradient(counts.completed,total):'var(--neutral-soft)',technical=snapshot.settings.debugMode,center=technical?(counts.failed?'var(--danger)':counts.attention?'var(--attention)':counts.running?'var(--ink)':'#fff'):'#fff',label=total?(technical?step.label+' — '+total+' tasks: '+counts.completed+' completed, '+counts.running+' running, '+counts.attention+' paused, '+counts.failed+' interrupted, '+counts.unfinished+' unfinished':step.label+' — '+counts.completed+' of '+total+' completed'):step.label+' — 0 tasks';return {total,composition,center,label,signature:[composition,center,label].join('|')}}
...
byId('workspace').onchange=event=>{if(event.target.id==='debug-mode')return void updateDebugMode(event.target.checked);if(event.target.id==='opener')return void updateOpener(event.target.value);if(event.target.id==='active-project-days')return void updateViewSettings()};
...
async function updateViewSettings(){try{const activeProjectDays=Number(byId('active-project-days').value),response=await fetch('/api/settings/view',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({activeProjectDays})}),result=await response.json();if(!response.ok)throw new Error(result.error||'Could not update View settings.');await load();showActionStatus('View settings saved.','success')}catch(error){showActionStatus(error.message,'failure');await load()}}
...
import { sortBlobs, viewerComponentScript, workspaceOpenMenuMarkup } from "./ViewerComponents.ts";
import {
